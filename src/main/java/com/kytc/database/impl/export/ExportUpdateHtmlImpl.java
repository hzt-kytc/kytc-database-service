...
@Service("exportUpdateHtmlImpl")
public class ExportUpdateHtmlImpl extends AbstractExportService {

	@Override
	public StringBuffer toStr() {
		// TODO Auto-generated method stub
		this.fileName = "update.jsp";
		...
		detailHtml.append("<%@ page language=\"java\" contentType=\"text/html; charset=UTF-8\""+
              ...
              tab(0)+"<script type=\"text/javascript\" src=\"${pageContext.request.contextPath}/js/"+javaTableName1+"/update.js\"></script>"+
              tab(0)+"<div id=\"cms_"+htmlName+"_update_main_div\" style=\"width:100%;height:100%;\">"+
              tab(1)+"<input type=\"hidden\" name=\""+id+"\" value=\"${result.data."+id+"}\">");
		int size = 4;
		...
		for(ColumnDTO dto:page.getRows()){
			...
			if(dto.getDataType().equals("date")){
				detailHtml.append(tab(4)+"<input class=\"easyui-datebox\" value=\"${result.data."+dto.getJavaName()+"}\" name=\""+dto.getJavaName()+"\"/>"+tab(3)+"</td>");
			}else if(dto.getDataType().equals("datetime")||dto.getDataType().equals("timestamp")){
				detailHtml.append(tab(4)+"<input class=\"easyui-datetimebox\" value=\"${result.data."+dto.getJavaName()+"}\" name=\""+dto.getJavaName()+"\"/>"+tab(3)+"</td>");
			}else if(dto.getColumnName().equals("sort_num")){
				detailHtml.append(tab(4)+"<input class=\"easyui-numberbox\" name=\"sortNum\" value=\"${result.data."+dto.getJavaName()+"}\" data-options=\"min:1,required:true\">"+tab(3)+"</td>");
			}else{
				detailHtml.append(tab(4)+"<input name=\""+dto.getJavaName()+"\" value=\"${result.data."+dto.getJavaName()+"}\" class=\"easyui-textbox\"/>"+tab(3)+"</td>");
			}
			i+=2;
